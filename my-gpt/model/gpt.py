...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.position_embeddings = nn.Embedding(vocab_size,model_dim)
        self.t_blocks = nn.Sequential()
        for _ in range(num_blocks):
            self.t_blocks.append(self.TransformerBlock(model_dim,num_heads))
        self.ln = nn.LayerNorm(model_dim)
        self.vp = nn.Linear(model_dim,vocab_size)


    def forward(self, context: TensorType[int]) -> TensorType[float]:
        torch.manual_seed(0)
        # 1. Add token embeddings + position embeddings (use torch.arange for positions)
        embed = self.word_embeddings(context)
        positions = torch.arange(context.shape[1])
        embed = embed+self.position_embeddings(positions)
        output = self.ln(self.t_blocks(embed))
        logits = self.vp(output)
